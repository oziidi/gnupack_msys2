;;; text-adjust.el --- text linter for japanese paragraph.
;;
...
;; --------------------------------------------------------------------
;;; private functions, imported mell and modified
(defun mell:list-mapfunc (list &optional value)
  (while list
    (setq value (funcall '+ value (car list))
          list (cdr list)))
  value)
...
得られた範囲を対象にする."
  (interactive "P")
  (save-excursion
    (or (mell:transient-region-active-p)
        (mark-paragraph))
...
(defun text-adjust-hankaku-region (from to)
  ...
  (save-excursion
    (let ((tmp-table (text-adjust--copy-char-table char-code-property-table)))
      ...
      (mapc #'(lambda (c) (text-adjust--modify-char-table c nil))
...
(defun text-adjust-kutouten-read-rule (&optional forcep)
  "変換後の句読点を選択する."
  (interactive)
  (if (and text-adjust-rule-kutouten
           (not forcep)
        ...
        (while (re-search-forward regexp nil t)
          (let ((n 1) (m 0) right-string)
            ...
            (while (not (match-beginning n))
              (setq n (+ n 3 (mell:list-mapadd (nth m counts)))
                    m (1+ m)))
                                        ; 該当パターンと置換する
            (let* ((tmp n)
                   (total-counts
...
(defun text-adjust--make-rule-pattern (rule)
  (let ((regexp (mapconcat
        ...
        (counts (mapcar
                 (lambda (x)
                   (list
                    (mell:match-count-string "\\\\(" (nth 0 (car x)))
                    (mell:match-count-string "\\\\(" (nth 1 (car x)))
                    (mell:match-count-string "\\\\(" (nth 2 (car x)))))
                 rule)))
